...
ATS::ATS() : date(""), cityCode(""), cityName(""), duration(""), tariff(0.0), phoneNumber("") {}

ATS::ATS(const std::string& d, const std::string& code, const std::string& name,
...
std::string* ATS::getPhonesByTariff(const std::string& filename, double targetTariff, int& count) {
    ...
    std::string* phones = new std::string[count];
    std::ifstream file(filename);
    if (!file) {
        delete[] phones;
        count = 0;
